...
Chloe 2026-07-20 定義的核心需求原話：

> 「能夠因為 AI 的靈活而靈活的改變計劃。這才能跟需求相呼應啊。」

目前產品只做到一半：**AI 靈活，但入口是死的**。計畫由 GPT-5.6 產生、每次報到後重算，
但使用者只能在系統排定的那一刻回應，無法在任何其他時刻告訴它真實狀況。兩個缺口屬於
同一個主題，應一起規劃，不要當成孤立的待辦：

1. **使用者推**：主動記錄進度的入口。AI 會在計畫裡寫「每完成一次記錄 1／5」，但介面
   沒有任何地方能記；真實模式下「分享進度」與「開啟報到」都導向同一個 `CHECK_IN`
   視圖（`journey-workspace.tsx:720`），而該視圖只有在存在 pending 報到時才能回報。
   **資料模型已預留空間**：`GoalAttendanceEntrySchema` 的 `checkInId` 可為 null、
   status 已含 `PARTIAL`、另有 `countsTowardGoal` 可區分正式報到與中途記錄，
   因此實作不需要改變出勤契約的形狀，只需新增入口與「誰有權寫出勤」的決策。
2. **AI 拉**：週期性的協議檢視。`reviewFrequencyDays` 目前是死欄位——schema 定義它、
   onboarding 顯示它（「協議檢視頻率 · N 天」）、也存進支持協議，但**全專案沒有任何
   地方讀取它來觸發檢視**。使用者會預期 N 天後 AI 回頭做一次全局檢討，實際不會發生。

3. **真正的來電**（Chloe 2026-07-20 提出，方向待她最終確認）。她的原話：

   > 「拒絕一個鈴聲是很容易的，但是你要拒絕一個人打電話過來是很難的。」

   這不是語音功能，是承諾機制。產品的敵人是逃避，而滑掉一則通知的成本近乎為零；
   要掛掉一通已接起的電話、對著正在說話的聲音承認自己沒做，成本高得多。
   **現狀**：專案為純 PWA，`package.json` 無 Capacitor／Cordova，全專案無
   Service Worker、Push API 或任何通知程式碼，因此無法主動觸發來電——這是瀏覽器的
   硬邊界，不是實作疏漏。**但難的一半已經完成**：`gpt-realtime-2.1` 的 WebRTC 即時
   語音對話層已可運作，只是依 AGENTS.md 設計為 user-started。缺的是「它主動找人」
   這一段。路線分三層：①背景推播（Service Worker + Push + VAPID，約 4–6 小時，
   仍只是通知）②全螢幕來電（Capacitor 打包 + Android full-screen intent，約 1–2 天，
   即 Chloe 先前 Android Bot 達到的效果）③註冊為通話 App（ConnectionService，再 1–2 天，
   進入系統通話記錄）。**2026-07-20 決定不在本輪施工**：第 2 層需時遠超剩餘賽程，
   且會把提交形態從 PWA 變成需安裝 APK，風險大於加分。

兩者湊起來才是「活的計畫」：一個是使用者推它，一個是它主動拉使用者。
第三項則決定這個計畫有沒有重量——**這個產品的核心不是提醒，是壓力。**

**2026-07-20 決定：兩項皆不在本輪施工**，因為對提交無影響，且應在精神充足時動工。
一次性目標當初只設計「一天一次」的測試情境，未預期一天多次，這是當時的合理範圍，
不是失誤。
